...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param) {
  ...
  info_t info;
    ...
    case FIFO_INFO:
        ...
        break;
    case FIFO_RESIZE:
    	 unsigned int new_size = param; 
         unsigned char *temp_buf;
         unsigned int old_size, copy_len;

        printk(KERN_INFO "%s: pchar_ioctl() resize fifo.\n", THIS_MODULE->name); 
		old_size = kfifo_size(&buf);

        // allocate temp buffer of fifo length - kmalloc(), kifo_len()
		temp_buf = kmalloc(old_size, GFP_KERNEL);
		if(!temp_buf)
		{
			printk(KERN_ERR "%s : pchar_ioctl() unable to allocate memory for temprary buffer.\n", THIS_MODULE->name);
			return -ENOMEM;
		}
	   ...
	   if(ret != old_size)
	   {
		printk(KERN_ERR "%s : pchar_ioctl() failed to copy data from kfifo to temporary buffer.\n", THIS_MODULE->name);
		kfree(temp_buf);
		return -EFAULT;
        ...
        // allocate kfifo with new size - kfifo_alloc() with param
         ...
         if (ret != 0) {
         printk(KERN_ERR "%s: pchar_ioctl() failed to allocate new kfifo with size %u.\n", THIS_MODULE->name, new_size);
         kfree(temp_buf);
         return ret;
    }
        ...
        // copy temp buffer data back to fifo - kfifo_in()
		 ...
		 ret = kfifo_in(&buf, temp_buf, copy_len);
        if (ret != copy_len) {
        printk(KERN_ERR "%s: pchar_ioctl() failed to copy data from temporary buffer to new kfifo.\n", THIS_MODULE->name);
        kfifo_free(&buf); // Cleanup on failure
        kfree(temp_buf);
        return -EFAULT;
    }
